...
// ----- Mutators ------
//
void Flight::FlightNumber(int flight_number)
{
	if (FlightNumber_ > 0) {
		std::cerr << "Flight already assigned Number.  Cannot re-assign." << std::endl;
	}
	else {
		//
		//  Since you cannot cancel an unassigned flight, 
		//  then the flight would not be cancelled at this point.
		//  Therefore, it is valid to set the cancelstatus here.
		//
		FlightNumber_ = flight_number;
		CancelStatus_ = false;
	}
...
void Flight::PilotsName(std::string pilots_name)
{
	if (FlightNumber_ == 0) {
		std::cerr << "Flight not assigned Number. Cannot Assign Pilot." << std::endl;
	}
	else if (CancelStatus_) {
		std::cerr << "Flight Cancelled.  Cannot Assign Pilot." << std::endl;
	}
	else {
		PilotsName_ = pilots_name;
	}
